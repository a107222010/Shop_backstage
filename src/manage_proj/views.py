...
from manage_menu.models import ShopDetail
...
def sign_up(request):
    ...
    if request.method == "POST":
        ...
        match_account = zhPattern.search(account)
        match_password = zhPattern.search(password)
        if len(shop_phone) > 10 or len(shop_phone) < 10:
            messages.error(request, '電話號碼錯誤，請重新輸入')
        else:
            if match_account:
                messages.error(request, '帳號錯誤，請勿包含中文')
            else:
                if match_password:
                    messages.error(request, '密碼錯誤，請勿包含中文')
                else:
                    ...
                    messages.error(request, '已成功創建帳戶')
                    return redirect("/login/")
